 ...
 * @option_param    {string}   rotate        旋转按钮的选择器或者DOM对象
	...
	function photoClip(container, option) {
		...
		if ($rotate.length) {
			$rotate.click(function() {
				if (imgLoaded)
					rotateCW({x: $clipView.offset().left+clipWidth/2, y:$clipView.offset().top+clipHeight/2});
			});
		}
		...
		function resetScroll() {
			...
			myScroll.zoom(Math.min(1, document.body.clientWidth/imgWidth));
